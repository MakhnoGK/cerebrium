...
1. **One writer process.** Only this MCP server touches the DB file. No sidecar scripts, no second process, no direct sqlite3 CLI writes in tooling. (Read-only inspection via CLI is fine.) If a worker thread is ever used for embedding inference, only the main thread touches the DB. — The `cerebrium-daemon` is the *one* sanctioned exception and is not a "sidecar": it runs this same codebase (the same per-aggregate repositories), and the `worker_lease` elects it as the **sole** embedding-drain writer, so exactly one process ever drains the queue. The `cerebrium-stats` CLI opens the DB `readonly` and never writes. — **Amended 2026-08-07: several MCP server processes may share one store.** Each agent host (Claude Code, Codex, Antigravity, pi) spawns its own server, so running two hosts at once means two writers, and that is now allowed rather than tolerated. Measured on this schema: two servers, 120 interleaved `write` calls plus searches against one file finished in 246 ms with **zero errors**, p95 7 ms, max 15 ms, and every node landed and was searchable — WAL plus `busy_timeout=15000` and `src/db/retry.ts` serialize them. What the amendment does **not** relax: the `worker_lease` remains the gate on background writer roles (embedding drain, consolidation), so exactly one process ever runs those, and the writes themselves stay in the repositories. The cold-start race this measurement exposed — two processes migrating an empty store — is fixed in `runMigrations` by reading the ledger inside a `BEGIN IMMEDIATE` transaction. — **Amended 2026-08-20 (architecture B):** the daemon now opens the file from more than one thread, and a host may open it from none. Its **read pool** workers each hold their own connection, opened `readonly`, so several reads run genuinely concurrently — WAL allows any number of readers and better-sqlite3 is synchronous only per connection. The **embedding model** lives in a worker of its own that touches no database at all, which inverts the old clause here: the worker that holds the model has no connection, and the workers that hold connections have no model (a read-pool worker gets a provider that refuses to embed, after one that quietly loaded a model per worker cost a measured +224MB for a single hybrid search). An MCP server in **proxy mode** registers no database whatsoever and resolves the same use-case tokens over the socket, which is the direction of travel: the end state is that nothing but the daemon opens the file. What this does **not** relax: only the daemon's main thread ever *writes*, every pool worker is read-only and fails rather than writing, and the `worker_lease` still gates the background writer roles.
2. **Append-only revisions.** Never `UPDATE` or `DELETE` a `revisions` row. Current content = latest rev.
3. **No hard deletes.** Any table holding a record. Soft-delete via `invalidated_at` only. Superseded data stays queryable via `history=true`. — The **derived vector index** (`chunk_vec`, `code_vec`, `embedding_meta`) is the one sanctioned exception, because it records nothing: every row is recomputable from `chunks.text`, and the `chunks` row it derives from is still only ever soft-deleted. `dropVector` is the single place that exercises it. This does **not** extend to `chunks`, `revisions`, `nodes`, `edges`, or any mirror table.
...
- **Layers, innermost first. The direction is enforced by `no-restricted-imports` in `eslint.config.js` — a violation fails `npm run check`.**
  ...
  - `src/domain/ports/` — the interfaces + DI tokens the inner layers own (`Clock`, `EmbeddingProvider`, `ConsolidationProvider`). May import `@/core` only.
  ...
  - Adapters/infra — `src/db/`, `src/code/`, `src/embeddings/`, `src/consolidation/`, `src/runtime/`. Implement the ports; never import delivery. They provide *capabilities*, not orchestration: `src/code/` walks/parses/extracts/resolves, while the indexing use case that sequences them and writes lives in `CodeIndexService`.
...
- One directory per MCP tool under `src/presentation/mcp/tools/<name>/{index.ts,metadata.ts}`. `metadata.ts` exports `{ name: ToolName, description, schema: z.ZodRawShape }`; `index.ts` exports a `class XxxTool implements McpTool<Schema, Response>` with `getMetadata()` and `invoke(args)` (no ctx). The `@tool()` class decorator registers it transiently under `TOOL_TOKEN`; `Server` receives the whole set via `@injectAll(TOOL_TOKEN)` and composes `ToolOutputAdapter(AuditedTool(tool))` — `AuditedTool` satisfies invariant #7 by appending the `events` row for every call (success and failure), `ToolOutputAdapter` wraps the result into the MCP `{content}`/`{isError}` envelope. Adding a tool = a new directory + a side-effect import in `src/presentation/mcp/tools/index.ts` + a `ToolName`->`EventAction` entry in `adapters/tool-action.ts` (a total `Record`, so a missing one fails `tsc`). Tool input validation with Zod at the boundary; the repo layer assumes valid input.
- **Event detail is declarative, never a call in a handler.** A tool that wants more than a bare audit row implements the optional `describeEvent(args, result)` from the `McpTool` contract, returning one `ToolEvent`, a list of them (one row per repo, as `code_index` does), or `null`. It may override `session_id` when the call mints one (`session_start`). Telemetry the response must not carry travels on a module-private `Symbol` key — invisible to `JSON.stringify`, so it costs the agent nothing.
...
- `npm run agent:setup` — report what each agent host (Claude Code, Codex, Antigravity, pi) still needs to use Cerebrium as memory; `-- --apply` installs it, `-- --verify` proves it. Read-only without `--apply`, and it never touches the store. The doctrine it installs lives in `install/`; see `install/hosts.md` for the per-host surfaces.
...
Don't scaffold speculatively — the *(future)* columns already in the schema are the entire allowed footprint of not-yet-built features. **Shipped:** code indexing, external mirrors, the `worker_lease`/daemon, consolidation (with per-role model routing), and the clean-architecture layering above. The MS MARCO cross-encoder reranker was removed in #17 and is not coming back on this evidence. **Still deferred:** Tier-2 hard reclaim of dead mirrors (physical delete + VACUUM) — it needs a narrow amendment to invariant #3 (no hard deletes) limited to derived `mirror` nodes; do not implement it without that sign-off.
