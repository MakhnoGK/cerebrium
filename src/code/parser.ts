...
async function grammarFor(wasm: string): Promise<Language> {
...
export async function parse(wasm: string, source: string): Promise<Tree> {
  ...
  parser.setLanguage(await grammarFor(wasm));
  const tree = parser.parse(source);
  if (!tree) throw new Error(`tree-sitter returned no tree for a ${wasm} source`);
  return tree;
}
