...
export const schema = {
  ...
  content: z.string().min(1).describe("Markdown body. First non-heading line becomes the summary."),
...
export async function handler(ctx: Ctx, args: ToolArgs<typeof schema>) {
  ...
  const similar = kind === "semantic" ? await dedupProbe(ctx, args) : [];
  ...
  const reconcile =
    similar.length && ctx.consolidator.enabled && reconcilePosture() !== "off"
      ? await tryReconcile(ctx, args, similar)
      : null;
...
async function dedupProbe(ctx: Ctx, args: ToolArgs<typeof schema>): Promise<SimilarExisting[]> {
  try {
